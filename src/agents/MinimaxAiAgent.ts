...
// 評分函數：領地分數 + 可走格數
function evaluate(gameState: GameSnapshot, player: string): number {
...
// Minimax 主體
function minimax(state: GameSnapshot, depth: number, maximizing: boolean, player: string): number {
...
// 擺子階段：最大化可移動格數
function selectBestPlacingAction(gameState: GameSnapshot, actions: PlayerAction[]): PlayerAction {
...
// --- 自殺步判斷 ---
function isSuicideMove(gameState: GameSnapshot, action: PlayerAction, player: string): boolean {
