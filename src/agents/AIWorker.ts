...
import {
  minimax as minimaxUtil,
  selectBestPlacingAction as selectMinimaxBestPlacingActionUtil,
} from '../utils/minimaxHelpers'

const N_CONST = 7 // Board size for Zobrist etc.

// --- Devil AI Logic (Adapted from former DevilWorker.ts) ---
const DEVIL_MAX_THINKING_TIME = 1000
const zobristStoneDevil: bigint[][][] = [...Array(N_CONST)].map(() =>
  [...Array(N_CONST)].map(() => [rand64Devil(), rand64Devil()]),
)
const zobristWallLeftDevil: bigint[][] = [...Array(N_CONST)].map(() =>
  [...Array(N_CONST)].map(rand64Devil),
)
const zobristWallTopDevil: bigint[][] = [...Array(N_CONST)].map(() =>
  [...Array(N_CONST)].map(rand64Devil),
)
...
function evalStateDevil(state: GameSnapshot, me: Player): number {
  ...
  const potentialTerritoryValue =
    (myPotentialTerritoryCount - oppPotentialTerritoryCount) * POTENTIAL_TERRITORY_WEIGHT
...
function calculateDevilPlayingMove(
  state: GameSnapshot,
...
function searchRootDevil(state: GameSnapshot, depth: number, t0: number, limit: number) {
  ...
  let best: PlayerAction | null = null
  const me = state.turn
  const moves = getLegalActions(state).sort(
    (a, b) => actionHeuristicDevil(state, b, me) - actionHeuristicDevil(state, a, me),
  )
...
function calculateMinimaxPlayingMove(
  ...
  for (const action of candidateActions) {
    const score = minimaxUtil(
      applyAction(cloneGameState(gameState), action),
      MINIMAX_DEPTH - 1,
      false,
      gameState.turn,
    )
  ...
  return getRandomAction({
    legalActions: bestActions.length > 0 ? bestActions : legalActions,
  })!
}

// --- Killer AI Logic ---
function calculateKillerPlayingMove(
  gameState: GameSnapshot,
  legalActions: PlayerAction[],
): PlayerAction {
  const safeActions = legalActions.filter((a) => !isSuicideMove(gameState, a, gameState.turn))
  ...
  try {
    ...
    if (gameState.phase === 'placing') {
      ...
      if (placementActions.length === 0) {
        // Should ideally not happen if legalActions is not empty
        self.postMessage({
          error: 'No legal placement actions available in placing phase.',
        })
        return
      }
      switch (aiType) {
