...
describe('Game Store', () => {
  it('placeStone: 正確擺子與換手', () => {
    ...
    act(() => { result.current.placeStone(pos) })
  ...
  it('moveTo: 棋子移動與步數', () => {
    const { result } = renderHook(() => useGame())
    // 擺子到 (0,0)
    act(() => { result.current.placeStone({ x: 0, y: 0 }) })
    // 擺子到 (1,0)
    act(() => { result.current.placeStone({ x: 1, y: 0 }) })
    // 進入 playing
    for (let i = 2; i < PLAYER_LIST.length * STONES_PER_PLAYER; i++) {
      act(() => { result.current.placeStone({ x: (i % BOARD_SIZE), y: Math.floor(i / BOARD_SIZE) }) })
    }
    // 選擇 (0,0)
    act(() => { result.current.selectStone({ x: 0, y: 0 }) })
    // 移動到 (0,1)
    act(() => { result.current.moveTo({ x: 0, y: 1 }) })
  ...
  it('buildWall: 能建牆', () => {
    ...
    act(() => { result.current.selectStone({ x: 0, y: 0 }) })
    // 建上牆
    act(() => { result.current.buildWall({ x: 0, y: 0 }, 'top') })
  ...
  it('多步 undo/redo', () => {
    ...
    act(() => { result.current.placeStone({ x: 0, y: 0 }) })
    act(() => { result.current.placeStone({ x: 1, y: 0 }) })
    act(() => { result.current.placeStone({ x: 2, y: 0 }) })
    act(() => { result.current.undo() })
    act(() => { result.current.undo() })
    expect(result.current.board[0][0].stone).toBe(PLAYER_LIST[0])
    expect(result.current.board[0][1].stone).toBe(null)
  ...
  it('moveTo: 非法移動不會改變狀態', () => {
    ...
    act(() => { result.current.selectStone({ x: 0, y: 0 }) })
    // 嘗試移動到遠處
    act(() => { result.current.moveTo({ x: 6, y: 6 }) })
    // 位置未變
    expect(result.current.board[0][0].stone).toBe(PLAYER_LIST[0])
