 ...
 */
function extractPlayerBoard(
  board: readonly (readonly Cell[])[],
): readonly (readonly (Player | null)[])[] {
  return board.map((row) => row.map((cell) => cell?.stone ?? null))
}

/**
 * Calculate territory ownership based on bordering pieces
 */
export function calculateTerritoryOwnership(region: Region): Player | null {
  const { borderingCounts } = region

  const players = PLAYER_LIST.filter((player) => borderingCounts[player] > 0)
...
export function detectTerritoryCapture(gameState: {
  ...
  const captured = previousTerritoryMap
    ? hasTerritoryChanged(previousTerritoryMap, newTerritoryMap)
    : false
